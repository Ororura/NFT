...
contract NFT is ERC1155("") {
    ...
    function createAsset(uint _id, string memory _name, string memory _desc, string memory _img, uint _price, uint _releasedAmount, uint _dateCreate) public OnlyOwner {
        ...
        assets[_id] = Asset(_id, _name, _desc, _img, _price, _releasedAmount, _dateCreate);
        userAssets[msg.sender].push(Asset(_id, _name, _desc, _img, _price, _releasedAmount, _dateCreate));
    }
    ...
    function buyNFT(uint _id, uint _amount, uint _discount) external {
        ...
        userAssets[msg.sender].push(Asset(userAssets[msg.sender].length, assets[sells[_id].assetId].name, assets[sells[_id].assetId].desc, assets[sells[_id].assetId].img, _amount, sells[_id].price, assets[sells[_id].assetId].dateCreate));
